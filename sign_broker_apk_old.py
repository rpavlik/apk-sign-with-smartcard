...
"""Sign a broker APK/AAB using the old/original keys (RSA 2048, self signed)."""
...
def _sign_apk(apksigner, apk_release_dir: Path, env: dict[str, str]):
    ...
    out_path = apk_release_dir / "installable_runtime_broker-release-signed-old-key.apk"
...
def _sign_bundle(apksigner, bundle_release_dir: Path, env: dict[str, str]):
    ...
    out_path = (
        bundle_release_dir / "installable_runtime_broker-release-signed-old-key.aab"
    )
    assert unsigned_path.exists()
